...
)
{
    ...
    this->E_ = calcElField(cloud, td);
    const scalar Q = Qdens_ * this->volume();;
    this->FLorentz_ = E_ * Q;
...
template<class ParcelType>
template<class TrackCloudType>
const Foam::vector Foam::KinematicParcel<ParcelType>::calcElField
(
    TrackCloudType& cloud,
    trackingData& td
) const
{
    ...
    vector elField = forces.elField(p, ttd);
...
:
    ...
    tTurb_(p.tTurb_),
    UTurb_(p.UTurb_),
    Qdens_(p.Qdens_),		// E. Weiand - 18/09/2019 - electric particle charge
    E_(p.E_),			// E. Weiand - 30/10/2019 - electric field strength
    FLorentz_(p.FLorentz_),	// E. Weiand - 30/10/2019 - Lorentz force
    particleRe_(p.particleRe_),	// E. Weiand - 26/11/2019 - particle Re
    localMaxCo_(p.localMaxCo_)	// E. Weiand - 06/03/2020 - local max step fraction
{}
...
:
    ...
    E_(p.E_),			// E. Weiand - 30/10/2019 - electric field strength
    FLorentz_(p.FLorentz_),	// E. Weiand - 30/10/2019 - Lorentz force
    particleRe_(p.particleRe_),	// E. Weiand - 26/11/2019 - particleRe
    localMaxCo_(p.localMaxCo_)	// E. Weiand - 06/03/2020 - local max step fraction
{}
...
)
{
    ...
    // E. Weiand - 2020/03/06: modified to take minimum of maxCo and evaporation timescale
    const scalar maxCo = cloud.solution().maxCo();
    //const scalar localCo = max(localMaxCo_, 0.0);
    //const scalar maxCo = max(min(cloud.solution().maxCo(), localCo/trackTime*0.1), 1e-8);

//    if (localCo/trackTime*0.1 < cloud.solution().maxCo())
//	Pout << "localCo = " << localCo << endl;
...
)
{
	...
	// E. Weiand - 2019/11/05: Do apply the patchInteraction models to coupled boundaries
